...
if (!taskId || typeof taskId !== "string") { return res.status(400).json({ error: "Invalid or missing Apify Task ID." }); }

if (!token || typeof token !== "string") { return res.status(400).json({ error: "Invalid or missing Apify API Token." }); }

try { const response = await fetch( https://api.apify.com/v2/actor-tasks/${taskId}/runs?token=${token}&limit=1&status=SUCCEEDED );
